...
export async function createManualObjectAction(input: unknown) {
  ...
  const object: ManagedObject = { ...data, id: crypto.randomUUID(), position: fixture.objects.filter((item) => item.status === data.status).length, category: null, archivedAt: null, cancelledAt: null, checklist: [], unresolvedDependencies: 0, recurrence: null, occurrenceNote: null };
