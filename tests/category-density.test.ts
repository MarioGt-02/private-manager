...
const source='\uFEFFsep=\t\nID\tTipo carta\n101\tInformatica\n102\tLegname\n103\tSport\n104\t\n';
beforeAll(async()=>{pg=new PGlite();db=drizzle(pg,{schema});for(const file of ['0000_flaky_meggan','0001_object_archive_metadata','0002_object_category_presentation','0003_silent_groot','0004_keen_diamondback','0005_boring_quicksilver'])await pg.exec(await readFile(`drizzle/${file}.sql`,'utf8'));},30000);
