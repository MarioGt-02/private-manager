...
export function ArchivedRow({ object, onOpen, onRestore, disabled }: { object: ArchivedObjectSummary; onOpen: () => void; onRestore: () => void; disabled: boolean }) {
...
export function ArchiveDrawer({ status, version, onClose, onOpen, onRestore }: {
  status: ObjectStatus; version: number; onClose: () => void; onOpen: (id: string) => Promise<void>; onRestore: (id: string) => Promise<void>;
}) {
  ...
  return <WorkspaceDialog label={`Archived — ${COLUMNS.find((column) => column.id === status)?.label}`} onClose={onClose}>
    ...
    <div className="min-h-0 flex-1 overflow-y-auto p-5">
      ...
      <ArchiveList key={`${status}:${filter}:${version}`} status={status} filter={filter} onOpen={onOpen} onRestore={onRestore} />
    </div>
  </WorkspaceDialog>;
}
function ArchiveList({ status, filter, onOpen, onRestore }: { status: ObjectStatus; filter: string; onOpen: (id: string) => Promise<void>; onRestore: (id: string) => Promise<void> }) {
  ...
  return <div aria-busy={loading}>
    ...
    {page && <><ul>{page.objects.map((object) => <ArchivedRow key={object.id} object={object} disabled={loading} onOpen={() => void perform(() => onOpen(object.id))} onRestore={() => void perform(() => onRestore(object.id))} />)}</ul>
