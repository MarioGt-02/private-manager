  ...
  return <WorkspaceDialog label="Object details" size="workspace" onClose={onClose} busy={pending}>
    <header className="flex shrink-0 flex-col gap-2 border-b border-slate-200 px-5 py-3 sm:px-6">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <span className="inline-flex items-center rounded-full border border-slate-200 bg-slate-50 px-2.5 py-0.5 text-xs font-medium text-slate-600">{column?.emoji} {column?.label}</span>
    ...
    <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain lg:grid lg:grid-cols-[minmax(0,1fr)_250px] lg:overflow-hidden xl:grid-cols-[minmax(0,1fr)_280px]">
      <section aria-label="Object content" className="min-h-0 min-w-0 px-5 py-4 sm:px-6 lg:overflow-y-auto lg:py-5">
        ...
        <div className="mt-3">
          <InlineEditableField label="Goal" value={object.goal} multiline emptyText="+ Add a goal" maxLength={1000} disabled={pending || archived} onSave={(value) => manual(() => onEditField(object.id, "goal", value))} />
        </div>
      ...
      <aside className="min-h-0 border-t border-slate-200 px-5 py-4 sm:px-6 lg:overflow-y-auto lg:border-l lg:border-t-0 lg:px-4 lg:py-5">
        <ActivityLog objectId={object.id} version={activityVersion} variant="panel" />
      </aside>
    </div>
  </WorkspaceDialog>;
}
